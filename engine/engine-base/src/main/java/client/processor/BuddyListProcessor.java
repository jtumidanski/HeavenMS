...
public class BuddyListProcessor {
   ...
   protected void getBuddies(int characterId, Consumer<Stream<Integer>> consumer) {
      UriBuilder.service(RestService.BUDDY).path("characters").path(characterId).path("buddies").getRestClient(GetBuddiesResponse.class)
            ...
            .failure(responseCode -> LoggerUtil.printError(LoggerOriginator.ENGINE, LogType.BUDDY_ORCHESTRATOR, "Failed to get buddies for character " + characterId))
            .get();
   }
   ...
   public void getBuddyListCapacity(int characterId, Consumer<Integer> consumer) {
      UriBuilder.service(RestService.BUDDY).path("characters").path(characterId).getRestClient(Character.class)
            ...
            .failure(responseCode -> LoggerUtil.printError(LoggerOriginator.ENGINE, LogType.BUDDY_ORCHESTRATOR, "Failed to get buddy list capacity for character " + characterId))
            .get();
   }
   ...
   public void loadBuddies(MapleCharacter character) {
      ...
      UriBuilder.service(RestService.BUDDY).path("characters").path(character.getId()).path("buddies").getRestClient(GetBuddiesResponse.class)
            ...
            .failure(responseCode -> LoggerUtil.printError(LoggerOriginator.ENGINE, LogType.BUDDY_ORCHESTRATOR, "Failed to load buddies for character " + character.getId()))
            .get();
   }
   ...
   public void syncCharacter(int accountId, int characterId) {
      syncCharacter(accountId, characterId, LambdaNoOp::doNothing, () -> LoggerUtil.printError(LoggerOriginator.ENGINE, LogType.BUDDY_ORCHESTRATOR, "Failed to sync character " + characterId));
   }
   ...
   public void deleteCharacter(int worldId, int characterId) {
      UriBuilder.service(RestService.BUDDY).path("characters").path(characterId).getRestClient()
            ...
            .failure(responseCode -> LoggerUtil.printError(LoggerOriginator.ENGINE, LogType.BUDDY_ORCHESTRATOR, "Failed to delete character " + characterId))
            .delete();
   }
   ...
   public void deleteBuddies(int characterId) {
      UriBuilder.service(RestService.BUDDY).path("characters").path(characterId).path("buddies").getRestClient()
            .failure(responseCode -> LoggerUtil.printError(LoggerOriginator.ENGINE, LogType.BUDDY_ORCHESTRATOR, "Failed to delete buddies for character " + characterId))
            .delete();
   }
