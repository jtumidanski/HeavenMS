...
class Quest22000 {
   ...
   def start(Byte mode, Byte type, Integer selection) {
      ...
      if (status == 0) {
         ...
         qm.sendNextPrev("#bYes, what about you, Mom?", (byte) 2)
      } else if (status == 2) {
         ...
         qm.sendNextPrev("#bOh, no. It's not that, Mom. I just had a strange dream last night.", (byte) 2)
      } else if (status == 4) {
         qm.sendNextPrev("A strange dream? What kind of strange dream?")
      } else if (status == 5) {
         qm.sendNextPrev("#bWell...", (byte) 2)
      } else if (status == 6) {
         qm.sendNextPrev("#b(You explain that you met a dragon in your dream.)", (byte) 2)
      } else if (status == 7) {
...
Quest22000 getQuest() {
   if (!getBinding().hasVariable("quest")) {
      ...
      getBinding().setVariable("quest", new Quest22000(qm: qm))
   }
   return (Quest22000) getBinding().getVariable("quest")
}
