...
class EventCafePQ_5 {
   ...
   int entryMap = 196000000
   int exitMap = 193000000
   int recruitMap = 193000000
   int clearMap
   int minMapId = 196000000
   int maxMapId = 196010000
   int eventTime = 45
   int[] lobbyRange = [0, 0]

   int couponsNeeded = 500
   int[] eventMaps = [196000000, 196010000]
   ...
   static def setEventRewards(EventInstanceManager eim) {
      ...
      List<Integer> itemSet = [4001012]
      List<Integer> itemQty = [1]
      eim.setEventRewards(evLevel, itemSet, itemQty)

      List<Integer> expStages = [25000]    //bonus exp given on CLEAR stage signal
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Lan5_" + lobbyid)
...
EventCafePQ_5 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventCafePQ_5(em: em))
   }
   return (EventCafePQ_5) getBinding().getVariable("event")
}
