...
class Event3rdJob_magician {
   ...
   int entryMap = 108010200
   int exitMap = 100040106
   int recruitMap
   int clearMap
   int minMapId = 108010200
   int maxMapId = 108010201
   int eventTime = 20
   int[] lobbyRange = [0, 7]
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("3rdJob_magician_" + lobbyid)
...
Event3rdJob_magician getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new Event3rdJob_magician(em: em))
   }
   return (Event3rdJob_magician) getBinding().getVariable("event")
}
