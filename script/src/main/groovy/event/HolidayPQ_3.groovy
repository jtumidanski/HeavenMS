...
class EventHolidayPQ_3 {
   EventManager em
   boolean isPq = true
   int minPlayers = 3, maxPlayers = 6
   int minLevel = 41, maxLevel = 50
   int entryMap = 889100021
   int exitMap = 889100022
   int recruitMap = 889100020
   int clearMap = 889100022
   int minMapId = 889100021
   int maxMapId = 889100021
   int eventTime = 25
   int[] lobbyRange = [0, 0]
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Holiday3_" + lobbyId)
   ...
   def snowmanEvolve(EventInstanceManager eim, int curLevel) {
      MapleMap map = eim.getInstanceMap(entryMap)
      int difficulty = eim.getIntProperty("level")
      MapleMonster snowman = map.getMonsterById(9400317 + (5 * difficulty) + (curLevel - 1))

      eim.setIntProperty("snowmanLevel", curLevel + 2)   // increment by 2 to decrement by 1 on friendlyKilled
      map.killMonster(snowman, null, false, 2)

      MapleLifeFactory.getMonster(9400317 + (5 * difficulty) + curLevel).ifPresent({ monster -> map.spawnMonsterOnGroundBelow(monster, new Point(-180, 15)) })
   ...
   def snowmanSnack(EventInstanceManager eim) {
      ...
      if (step >= 2 + (eim.getIntProperty("level") * snowmanLevel)) {
         ...
         MapleMap map = eim.getInstanceMap(entryMap)
         int difficulty = eim.getIntProperty("level")
         MapleMonster snowman = map.getMonsterById(9400316 + (5 * difficulty) + snowmanLevel)
   ...
   static def snowmanSnackFake(EventInstanceManager eim) {
      ...
      MessageBroadcaster.getInstance().sendServerNotice(eim.getPlayers(), ServerNoticeType.PINK_TEXT, I18nMessage.from("HENESYS_PQ_SNOWMAN_ABSORB"))
   }
...
EventHolidayPQ_3 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventHolidayPQ_3(em: em))
   }
   return (EventHolidayPQ_3) getBinding().getVariable("event")
}
