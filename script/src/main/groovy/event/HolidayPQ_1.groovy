...
class EventHolidayPQ_1 {
   EventManager em
   boolean isPq = true
   int minPlayers = 3, maxPlayers = 6
   int minLevel = 21, maxLevel = 30
   int entryMap = 889100001
   int exitMap = 889100002
   int recruitMap = 889100000
   int clearMap = 889100002
   int minMapId = 889100001
   int maxMapId = 889100001
   int eventTime = 15
   int[] lobbyRange = [0, 0]
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Holiday1_" + lobbyId)
...
EventHolidayPQ_1 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventHolidayPQ_1(em: em))
   }
   return (EventHolidayPQ_1) getBinding().getVariable("event")
}
