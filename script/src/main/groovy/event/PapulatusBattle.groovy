...
class EventPapulatusBattle {
   EventManager em
   boolean isPq = true
   int minPlayers = 1, maxPlayers = 6
   int minLevel = 1, maxLevel = 255
   int entryMap = 220080001
   int exitMap = 220080000
   int recruitMap = 220080000
   int clearMap = 220080000
   int minMapId = 220080001
   int maxMapId = 220080001
   int eventTime = 45
   int[] lobbyRange = [0, 0]
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Papulatus" + lobbyId)
   ...
   def updateGateState(byte newState) {
...
EventPapulatusBattle getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventPapulatusBattle(em: em))
   }
   return (EventPapulatusBattle) getBinding().getVariable("event")
}
