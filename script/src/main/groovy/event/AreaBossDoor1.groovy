...
class EventAreaBossDoor1 {
   ...
   def start() {
      int bossMobId = 9400610
      int bossMapId = 677000003
      Point bossPos = new Point(467, 0)
      ...
      MapleLifeFactory.getMonster(bossMobId).ifPresent({ boss ->
         ...
         MessageBroadcaster.getInstance().sendMapServerNotice(map, ServerNoticeType.LIGHT_BLUE, I18nMessage.from("AMDUSIAS_SUMMONED"))
         em.schedule("start", 3 * 60 * 60 * 1000)
      })
   }
...
EventAreaBossDoor1 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossDoor1(em: em))
   }
   return (EventAreaBossDoor1) getBinding().getVariable("event")
}
