...
class EventCygnus_Magic_Library {
   ...
   int entryMap = 101000000
   int exitMap = 101000000
   int recruitMap
   int clearMap
   int minMapId
   int maxMapId
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Cygnus_Magic_Library_" + lobbyid)
   ...
   def playerEntry(EventInstanceManager eim, MapleCharacter player) {
      MapleMap magicLibrary = eim.getMapInstance(eventMap)
...
EventCygnus_Magic_Library getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventCygnus_Magic_Library(em: em))
   }
   return (EventCygnus_Magic_Library) getBinding().getVariable("event")
}
