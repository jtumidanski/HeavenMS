...
class EventAran_3rdmount {
   EventManager em
   boolean isPq = true
   int minPlayers, maxPlayers
   int minLevel, maxLevel
   int entryMap = 914030000
   int exitMap = 140010210
   int recruitMap
   int clearMap
   int minMapId = 914030000
   int maxMapId = 914030000
   int eventTime = 3
   int[] lobbyRange = [0, 7]
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Aran_3rdmount_" + lobbyid)
...
EventAran_3rdmount getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAran_3rdmount(em: em))
   }
   return (EventAran_3rdmount) getBinding().getVariable("event")
}
