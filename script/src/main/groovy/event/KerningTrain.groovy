...
class EventKerningTrain {
   ...
   int[] returnTo = [103000100, 103000310]
   int[] rideTo = [103000310, 103000100]
   int[] trainRide = [103000301, 103000302]
   int myRide
   MapleMap returnMap
   MapleMap map
   MapleMap onRide

   int rideTime = 10 * 1000
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("KerningTrain_" + lobbyId)
      return eim
   }
   ...
   def playerEntry(EventInstanceManager eim, MapleCharacter player) {
      ...
      PacketCreator.announce(player, new ShowTitleEarned("The next stop is at Kerning " + (myRide == 0 ? "Square" : "Subway") + " Station. The exit is to your left."))
   ...
   def playerExit(EventInstanceManager eim, MapleCharacter player, boolean success) {
...
EventKerningTrain getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventKerningTrain(em: em))
   }
   return (EventKerningTrain) getBinding().getVariable("event")
}
...
def playerExit(EventInstanceManager eim, MapleCharacter player, boolean success) {
   getEvent().playerExit(eim, player, success)
}
