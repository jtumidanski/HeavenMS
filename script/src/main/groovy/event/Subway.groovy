...
class EventSubway {
   ...
   MapleMap KC_Waiting
   MapleMap Subway_to_KC
   MapleMap KC_docked
   MapleMap NLC_Waiting
   MapleMap Subway_to_NLC
   MapleMap NLC_docked

//Time Setting is in millisecond
   int closeTime = 50 * 1000 //The time to close the gate
   int beginTime = 1 * 60 * 1000 //The time to begin the ride
   int rideTime = 4 * 60 * 1000 //The time that require move to destination
   ...
   def takeoff() {
      ...
      MasterBroadcaster.getInstance().sendToAllInMap(KC_docked, new PlaySound("subway/whistle"))
      MasterBroadcaster.getInstance().sendToAllInMap(NLC_docked, new PlaySound("subway/whistle"))
...
EventSubway getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventSubway(em: em))
   }
   return (EventSubway) getBinding().getVariable("event")
}
