...
class EventCafePQ_3 {
   ...
   int entryMap = 192000000
   int exitMap = 193000000
   int recruitMap = 193000000
   int clearMap
   int minMapId = 192000000
   int maxMapId = 192000001
   int eventTime = 45
   int[] lobbyRange = [0, 0]

   int couponsNeeded = 350
   int[] eventMaps = [192000000, 192000001]
   ...
   static def setEventRewards(EventInstanceManager eim) {
      ...
      List<Integer> itemSet = [4001013]
      List<Integer> itemQty = [1]
      eim.setEventRewards(evLevel, itemSet, itemQty)

      List<Integer> expStages = [12000]    //bonus exp given on CLEAR stage signal
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Lan3_" + lobbyid)
...
EventCafePQ_3 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventCafePQ_3(em: em))
   }
   return (EventCafePQ_3) getBinding().getVariable("event")
}
