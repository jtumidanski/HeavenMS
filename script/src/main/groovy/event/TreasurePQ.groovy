...
class EventTreasurePQ {
   EventManager em
   boolean isPq = true
   int minPlayers = 4, maxPlayers = 6
   int minLevel = 140, maxLevel = 255
   int entryMap = 674030000
   int exitMap = 674030100
   int recruitMap = 674030100
   int clearMap = 674030300
   int minMapId = 674030000
   int maxMapId = 674030300
   int eventTime = 45
   int[] lobbyRange = [0, 0]
   ...
   static def setEventRewards(EventInstanceManager eim) {
      ...
      List<Integer> expStages = [60000, 100000]    //bonus exp given on CLEAR stage signal
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Treasure" + lobbyId)
   ...
   def warpBonus(EventInstanceManager eim) {
...
EventTreasurePQ getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventTreasurePQ(em: em))
   }
   return (EventTreasurePQ) getBinding().getVariable("event")
}
