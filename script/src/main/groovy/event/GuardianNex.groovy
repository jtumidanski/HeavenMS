...
class EventGuardianNex {
   EventManager em
   boolean isPq = true
   int minPlayers = 1, maxPlayers
   int minLevel, maxLevel
   int entryMap
   int exitMap = 240070000
   int recruitMap
   int clearMap
   int minMapId
   int maxMapId
   ...
   int eventMap = 240070010
   int[] eventBossIds = [7120100, 7120101, 7120102, 8120100, 8120101, 8140510]
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Nex_" + lobbyid)
...
EventGuardianNex getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventGuardianNex(em: em))
   }
   return (EventGuardianNex) getBinding().getVariable("event")
}
