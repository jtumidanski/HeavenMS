...
class EventCafePQ_1 {
   ...
   int entryMap = 190000000
   int exitMap = 193000000
   int recruitMap = 190000000
   int clearMap
   int minMapId = 190000000
   int maxMapId = 190000002
   int eventTime = 45
   int[] lobbyRange = [0, 0]

   int couponsNeeded = 400
   int[] eventMaps = [190000000, 190000001, 190000002]
   ...
   static def setEventRewards(EventInstanceManager eim) {
      ...
      List<Integer> itemSet = [4001014]
      List<Integer> itemQty = [1]
      eim.setEventRewards(evLevel, itemSet, itemQty)

      List<Integer> expStages = [20000]    //bonus exp given on CLEAR stage signal
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Lan1_" + lobbyid)
...
EventCafePQ_1 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventCafePQ_1(em: em))
   }
   return (EventCafePQ_1) getBinding().getVariable("event")
}
