...
class EventMK_PrimeMinister2 {
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("MK_PrimeMinister2_" + lobbyId)
      respawn(eim)
   ...
   def respawn(EventInstanceManager eim) {
      if (primeMinisterCheck(eim)) {
         ...
         MapleLifeFactory.getMonster(mobId).ifPresent({ monster -> weddingHall.spawnMonsterOnGroundBelow(monster, new Point(292, 143)) })
      } else {
...
EventMK_PrimeMinister2 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventMK_PrimeMinister2(em: em))
   }
   return (EventMK_PrimeMinister2) getBinding().getVariable("event")
}
