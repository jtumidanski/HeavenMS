...
class EventKerningPQ {
   EventManager em
   boolean isPq = true
   int minPlayers = 3, maxPlayers = 4
   int minLevel = 21, maxLevel = 30
   int entryMap = 103000800
   int exitMap = 103000890
   int recruitMap = 103000000
   int clearMap = 103000805
   int minMapId = 103000800
   int maxMapId = 103000805
   int eventTime = 30
   int[] lobbyRange = [0, 0]
   ...
   static def setEventExclusives(EventInstanceManager eim) {
      List<Integer> itemSet = [4001007, 4001008]
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Kerning" + lobbyId)
...
EventKerningPQ getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventKerningPQ(em: em))
   }
   return (EventKerningPQ) getBinding().getVariable("event")
}
