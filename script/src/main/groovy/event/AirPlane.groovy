...
EventAirPlane getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAirPlane(em: em))
   }
   return (EventAirPlane) getBinding().getVariable("event")
}
