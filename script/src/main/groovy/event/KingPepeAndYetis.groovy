...
class EventKingPepeAndYetis {
   EventManager em
   boolean isPq = true
   int minPlayers = 1, maxPlayers
   int minLevel, maxLevel
   int entryMap
   int exitMap = 106021400
   int recruitMap
   int clearMap
   int minMapId
   int maxMapId
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("KingPepe_" + lobbyId)
   ...
   def playerEntry(EventInstanceManager eim, MapleCharacter player) {
      MapleMap yetiMap = eim.getMapInstance(eventMap)
...
EventKingPepeAndYetis getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventKingPepeAndYetis(em: em))
   }
   return (EventKingPepeAndYetis) getBinding().getVariable("event")
}
