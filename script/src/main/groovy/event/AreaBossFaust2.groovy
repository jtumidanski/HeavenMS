...
class EventAreaBossFaust2 {
   ...
   def start() {
      MapleMap theForestOfEvil2 = em.getChannelServer().getMapFactory().getMap(100040106)
      MapleMonster faust2 = MapleLifeFactory.getMonster(5220002)
...
EventAreaBossFaust2 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossFaust2(em: em))
   }
   return (EventAreaBossFaust2) getBinding().getVariable("event")
}
