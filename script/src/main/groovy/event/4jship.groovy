...
class Event4jship {
   ...
   int entryMap = 912010000
   int exitMap = 120000101
   int recruitMap
   int clearMap
   int minMapId = 912010000
   ...
   static def playerCanLeave(EventInstanceManager eim) {
   ...
   static def playerSurvived(EventInstanceManager eim) {
      if (eim.getLeader().isAlive()) {
         ...
         MessageBroadcaster.getInstance().sendServerNotice(eim.getPlayers(), ServerNoticeType.PINK_TEXT, I18nMessage.from("KYRIN_TEST_PASS"))
      } else {
         MessageBroadcaster.getInstance().sendServerNotice(eim.getPlayers(), ServerNoticeType.PINK_TEXT, I18nMessage.from("KYRIN_TEST_FAIL"))
      }
   }
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("4jship_" + lobbyId)
...
Event4jship getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new Event4jship(em: em))
   }
   return (Event4jship) getBinding().getVariable("event")
}
