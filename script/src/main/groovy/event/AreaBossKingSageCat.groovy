...
class EventAreaBossKingSageCat {
   ...
   def start() {
      MapleMap goblinForest2 = em.getChannelServer().getMapFactory().getMap(250010504)
      MapleMonster kingSageCat = MapleLifeFactory.getMonster(7220002)
...
EventAreaBossKingSageCat getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossKingSageCat(em: em))
   }
   return (EventAreaBossKingSageCat) getBinding().getVariable("event")
}
