...
class EventAreaBossDoor6 {
   ...
   def start() {
      int bossMobId = 9400611
      int bossMapId = 677000007
      Point bossPos = new Point(171, 50)
      ...
      MapleLifeFactory.getMonster(bossMobId).ifPresent({ boss ->
         ...
         MessageBroadcaster.getInstance().sendMapServerNotice(map, ServerNoticeType.LIGHT_BLUE, I18nMessage.from("CROCELL_SUMMONED"))
         em.schedule("start", 3 * 60 * 60 * 1000)
      })
   }
...
EventAreaBossDoor6 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossDoor6(em: em))
   }
   return (EventAreaBossDoor6) getBinding().getVariable("event")
}
