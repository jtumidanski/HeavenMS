...
class EventHorntailPQ {
   EventManager em
   boolean isPq = true
   int minPlayers = 6, maxPlayers = 6
   int minLevel = 120, maxLevel = 255
   int entryMap = 240050100
   int exitMap = 240050000
   int recruitMap = 240050000
   int clearMap = 240050400
   int minMapId = 240050100
   int maxMapId = 240050310
   int eventTime = 25
   int[] lobbyRange = [0, 0]
   ...
   static def setEventExclusives(EventInstanceManager eim) {
      List<Integer> itemSet = [4001087, 4001088, 4001089, 4001090, 4001091, 4001092, 4001093]
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("PreHorntail" + lobbyId)
      ...
      eim.getInstanceMap(240050101).getReactorByName("passKey1").setEventState((byte) 0)
      eim.getInstanceMap(240050102).getReactorByName("passKey2").setEventState((byte) 1)
      eim.getInstanceMap(240050103).getReactorByName("passKey3").setEventState((byte) 2)
      eim.getInstanceMap(240050104).getReactorByName("passKey4").setEventState((byte) 3)
...
EventHorntailPQ getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventHorntailPQ(em: em))
   }
   return (EventHorntailPQ) getBinding().getVariable("event")
}
