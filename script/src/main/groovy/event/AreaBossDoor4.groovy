...
EventAreaBossDoor4 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossDoor4(em: em))
   }
   return (EventAreaBossDoor4) getBinding().getVariable("event")
}
