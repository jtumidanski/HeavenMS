...
EventAreaBossDoor5 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossDoor5(em: em))
   }
   return (EventAreaBossDoor5) getBinding().getVariable("event")
}
