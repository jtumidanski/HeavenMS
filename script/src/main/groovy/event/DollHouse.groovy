...
class EventDollHouse {
   ...
   int entryMap = 922000010
   int exitMap = 221024400
   int recruitMap
   int clearMap
   int minMapId
   int maxMapId
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("DollHouse_" + lobbyid)
...
EventDollHouse getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventDollHouse(em: em))
   }
   return (EventDollHouse) getBinding().getVariable("event")
}
