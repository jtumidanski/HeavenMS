...
class Event3rdJob_pirate {
   EventManager em
   boolean isPq = true
   int minPlayers, maxPlayers
   int minLevel, maxLevel
   int entryMap = 108010500
   int exitMap = 105070200
   int recruitMap
   int clearMap
   int minMapId = 108010500
   int maxMapId = 108010501
   int eventTime = 20
   int[] lobbyRange = [0, 7]
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("3rdJob_pirate_" + lobbyId)
...
Event3rdJob_pirate getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new Event3rdJob_pirate(em: em))
   }
   return (Event3rdJob_pirate) getBinding().getVariable("event")
}
