...
class EventAreaBossNineTailedFox {
   EventManager em
   boolean isPq = true
   ...
   def start() {
      MapleMap moonRidge = em.getChannelServer().getMapFactory().getMap(222010310)
      MapleMonster nineTailedFox = MapleLifeFactory.getMonster(7220001)
...
EventAreaBossNineTailedFox getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossNineTailedFox(em: em))
   }
   return (EventAreaBossNineTailedFox) getBinding().getVariable("event")
}
