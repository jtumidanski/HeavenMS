...
class EventHenesysPQ {
   ...
   int minLevel = 10, maxLevel = 255
   int entryMap = 910010000
   int exitMap = 910010300
   int recruitMap = 100000200
   int clearMap = 910010100
   int minMapId = 910010000
   int maxMapId = 910010400
   int eventTime = 10
   int[] lobbyRange = [0, 0]
   ...
   static def setEventExclusives(EventInstanceManager eim) {
      List<Integer> itemSet = [4001095, 4001096, 4001097, 4001098, 4001099, 4001100, 4001101]
   ...
   static def setEventRewards(EventInstanceManager eim) {
      ...
      List<Integer> itemSet = [4001158]
      List<Integer> itemQty = [1]
      eim.setEventRewards(evLevel, itemSet, itemQty)

      List<Integer> expStages = [1600]    //bonus exp given on CLEAR stage signal
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Henesys" + lobbyId)
   ...
   def scheduledTimeout(EventInstanceManager eim) {
      if (eim.getProperty("1stageclear") != null) {
         int curStage = 910010200, toStage = 910010400
   ...
   def bunnyDefeated(EventInstanceManager eim) {
      MessageBroadcaster.getInstance().sendServerNotice(eim.getPlayers(), ServerNoticeType.PINK_TEXT, I18nMessage.from("HENESYS_PQ_FAIL"))
      end(eim)
   }
...
EventHenesysPQ getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventHenesysPQ(em: em))
   }
   return (EventHenesysPQ) getBinding().getVariable("event")
}
