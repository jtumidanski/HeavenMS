...
class EventAreaBossTimer1 {
   ...
   def start() {
      MapleMap whirlpoolOfTime = em.getChannelServer().getMapFactory().getMap(220050100)
      MapleMonster timer1 = MapleLifeFactory.getMonster(5220003)
      ...
      int posX
      int posY = 1030
      posX = Math.floor((Math.random() * 770) - 770).intValue()
      whirlpoolOfTime.spawnMonsterOnGroundBelow(timer1, new Point(posX, posY))
      MessageBroadcaster.getInstance().sendMapServerNotice(whirlpoolOfTime, ServerNoticeType.LIGHT_BLUE,"Tick-Tock Tick-Tock! Timer makes it's presence known.")
      em.schedule("start", 3 * 60 * 60 * 1000)
   }
...
EventAreaBossTimer1 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossTimer1(em: em))
   }
   return (EventAreaBossTimer1) getBinding().getVariable("event")
}
