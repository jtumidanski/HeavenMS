...
class EventAreaBossFaust1 {
   ...
   def start() {
      MapleMap theForestOfEvil1 = em.getChannelServer().getMapFactory().getMap(100040105)
      MapleMonster faust1 = MapleLifeFactory.getMonster(5220002)
      ...
      MessageBroadcaster.getInstance().sendMapServerNotice(theForestOfEvil1, ServerNoticeType.LIGHT_BLUE, "Faust appeared amidst the blue fog.")
      em.schedule("start", 3 * 60 * 60 * 1000)
   }
...
EventAreaBossFaust1 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossFaust1(em: em))
   }
   return (EventAreaBossFaust1) getBinding().getVariable("event")
}
