...
class Event3rdJob_thief {
   EventManager em
   boolean isPq = true
   int minPlayers, maxPlayers
   int minLevel, maxLevel
   int entryMap = 108010400
   int exitMap = 107000402
   int recruitMap
   int clearMap
   int minMapId = 108010400
   int maxMapId = 108010401
   int eventTime = 20
   int[] lobbyRange = [0, 7]
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("3rdJob_thief_" + lobbyId)
...
Event3rdJob_thief getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new Event3rdJob_thief(em: em))
   }
   return (Event3rdJob_thief) getBinding().getVariable("event")
}
