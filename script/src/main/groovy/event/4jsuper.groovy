...
class Event4jsuper {
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("4jsuper_" + lobbyId)
...
Event4jsuper getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new Event4jsuper(em: em))
   }
   return (Event4jsuper) getBinding().getVariable("event")
}
