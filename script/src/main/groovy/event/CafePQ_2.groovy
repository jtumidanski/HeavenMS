...
class EventCafePQ_2 {
   ...
   int entryMap = 191000000
   int exitMap = 193000000
   int recruitMap = 193000000
   int clearMap
   int minMapId = 191000000
   int maxMapId = 191000001
   int eventTime = 45
   int[] lobbyRange = [0, 0]

   int couponsNeeded = 350
   int[] eventMaps = [191000000, 191000001]
   ...
   static def setEventRewards(EventInstanceManager eim) {
      ...
      List<Integer> itemSet = [4001009]
      List<Integer> itemQty = [1]
      eim.setEventRewards(evLevel, itemSet, itemQty)

      List<Integer> expStages = [15000]    //bonus exp given on CLEAR stage signal
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Lan2_" + lobbyId)
...
EventCafePQ_2 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventCafePQ_2(em: em))
   }
   return (EventCafePQ_2) getBinding().getVariable("event")
}
