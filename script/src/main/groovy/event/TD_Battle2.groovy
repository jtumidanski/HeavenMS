...
class EventTD_Battle2 {
   EventManager em
   boolean isPq = true
   int minPlayers = 2, maxPlayers = 6
   int minLevel = 70, maxLevel = 255
   int entryMap = 240070303
   int exitMap = 240070302
   int recruitMap = 240070302
   int clearMap = 240070302
   int minMapId = 240070303
   int maxMapId = 240070303
   int eventTime = 10
   int[] lobbyRange = [0, 0]

   int tdBossId = 8220010
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("TDBoss" + lobbyId)
...
EventTD_Battle2 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventTD_Battle2(em: em))
   }
   return (EventTD_Battle2) getBinding().getVariable("event")
}
