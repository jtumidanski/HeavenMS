...
class EventAreaBossKingClang {
   ...
   def start() {
      ...
      MapleMonster kingClang = MapleLifeFactory.getMonster(5220001)
      int posX
      int posY = 140
...
EventAreaBossKingClang getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossKingClang(em: em))
   }
   return (EventAreaBossKingClang) getBinding().getVariable("event")
}
