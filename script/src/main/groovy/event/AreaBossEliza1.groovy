...
class EventAreaBossEliza1 {
   ...
   def start() {
      MapleMap stairwayToTheSky2 = em.getChannelServer().getMapFactory().getMap(200010300)
      MapleMonster eliza = MapleLifeFactory.getMonster(8220000)
...
EventAreaBossEliza1 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossEliza1(em: em))
   }
   return (EventAreaBossEliza1) getBinding().getVariable("event")
}
