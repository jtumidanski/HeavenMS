...
class EventAreaBossDeo {
   ...
   def start() {
      MapleMap royalCatthusDesert = em.getChannelServer().getMapFactory().getMap(260010201)
      MapleMonster deo = MapleLifeFactory.getMonster(3220001)
...
EventAreaBossDeo getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossDeo(em: em))
   }
   return (EventAreaBossDeo) getBinding().getVariable("event")
}
