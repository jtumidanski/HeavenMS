...
class EventWeddingChapel {
   EventManager em
   boolean isPq = true
   int minPlayers, maxPlayers
   int minLevel, maxLevel
   int entryMap = 680000100
   ...
   int startMsgTime = 4
   int blessMsgTime = 5
   int ceremonyTime = 20 // 20 minutes ceremony
   int blessingsTime = 15// blessings are held until the 15th minute from the ceremony start
   int partyTime = 45    // 45 minutes party

   int forceHideMsgTime = 10
   // unfortunately, EIM weddings don't send wedding talk packets to the server... this will need to suffice

   boolean eventBoss = true   // spawns a Cake boss at the hunting ground
   boolean isCathedral = false
...
EventWeddingChapel getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventWeddingChapel(em: em))
   }
   return (EventWeddingChapel) getBinding().getVariable("event")
}
