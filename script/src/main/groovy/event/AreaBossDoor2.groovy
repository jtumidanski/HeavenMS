...
class EventAreaBossDoor2 {
   ...
   def start() {
      int bossMobId = 9400609
      int bossMapId = 677000005
      Point bossPos = new Point(201, 80)
      ...
      MapleLifeFactory.getMonster(bossMobId).ifPresent({ boss ->
         ...
         MessageBroadcaster.getInstance().sendMapServerNotice(map, ServerNoticeType.LIGHT_BLUE, I18nMessage.from("ANDRAS_SUMMONED"))
         em.schedule("start", 3 * 60 * 60 * 1000)
      })
   }
...
EventAreaBossDoor2 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossDoor2(em: em))
   }
   return (EventAreaBossDoor2) getBinding().getVariable("event")
}
