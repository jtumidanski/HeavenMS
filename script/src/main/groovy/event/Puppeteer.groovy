...
class EventPuppeteer {
   EventManager em
   boolean isPq = true
   int minPlayers = 1, maxPlayers
   int minLevel, maxLevel
   int entryMap
   int exitMap = 105070300
   int recruitMap
   int clearMap
   int minMapId = 910510000
   int maxMapId = 910510000
   int eventTime
   int[] lobbyRange = [0, 0]

   int timeLimit = 1
   int eventTimer = 1000 * 60 * timeLimit
   int eventMap = 910510000
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Puppeteer_" + lobbyid)
   ...
   def playerEntry(EventInstanceManager eim, MapleCharacter player) {
      MapleMap cave = eim.getMapInstance(eventMap)
...
EventPuppeteer getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventPuppeteer(em: em))
   }
   return (EventPuppeteer) getBinding().getVariable("event")
}
