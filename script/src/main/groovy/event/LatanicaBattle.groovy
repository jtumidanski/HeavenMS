...
class EventLatanicaBattle {
   EventManager em
   boolean isPq = true
   int minPlayers = 1, maxPlayers = 6
   int minLevel = 1, maxLevel = 255
   int entryMap = 541010100
   int exitMap = 541010110
   int recruitMap = 541010060
   int clearMap = 541010110
   int minMapId = 541010100
   int maxMapId = 541010100
   int eventTime = 10
   int[] lobbyRange = [0, 0]
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Latanica" + lobbyId)
...
EventLatanicaBattle getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventLatanicaBattle(em: em))
   }
   return (EventLatanicaBattle) getBinding().getVariable("event")
}
