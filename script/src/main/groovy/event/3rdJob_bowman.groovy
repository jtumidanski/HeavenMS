...
class Event3rdJob_bowman {
   ...
   int entryMap = 108010100
   int exitMap = 105040305
   int recruitMap
   int clearMap
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("3rdJob_bowman_" + lobbyid)
...
Event3rdJob_bowman getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new Event3rdJob_bowman(em: em))
   }
   return (Event3rdJob_bowman) getBinding().getVariable("event")
}
