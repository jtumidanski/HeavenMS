...
class EventAran_2ndmount {
   ...
   int entryMap = 921110000
   int exitMap = 211050000
   int recruitMap
   int clearMap
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("Aran_2ndmount_" + lobbyId)
...
EventAran_2ndmount getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAran_2ndmount(em: em))
   }
   return (EventAran_2ndmount) getBinding().getVariable("event")
}
