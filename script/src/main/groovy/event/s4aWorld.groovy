...
class Events4aWorld {
   ...
   def setup(int level, int lobbyId) {
      EventInstanceManager eim = em.newInstance("s4aWorld_" + lobbyId)
   ...
   static def playerEntry(EventInstanceManager eim, MapleCharacter player) {
      MapleMap map = eim.getMapFactory().getMap(910500000)
   ...
   static def playerExit(EventInstanceManager eim, MapleCharacter player) {
      eim.unregisterPlayer(player)
      MapleMap map = eim.getMapFactory().getMap(105090200)
      player.changeMap(map, map.getPortal(0))
   }
   ...
   def changedMap(EventInstanceManager eim, MapleCharacter player, int mapId) {
      if (mapId != 910500000) {
         ...
         if (eim.disposeIfPlayerBelow((byte) minPlayers, 105090200)) {
            em.setProperty("started", "false")
         }
      }
   }
   ...
   def scheduledTimeout(EventInstanceManager eim) {
      eim.disposeIfPlayerBelow((byte) 100, 105090200)

      em.setProperty("started", "false")
   }

   static def timeOut(EventInstanceManager eim) {
   ...
   static def playerDisconnected(EventInstanceManager eim, MapleCharacter player) {
      return 0
   }
   ...
   static def leftParty(EventInstanceManager eim, MapleCharacter player) {
      playerExit(eim, player)
   }
...
Events4aWorld getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new Events4aWorld(em: em))
   }
   return (Events4aWorld) getBinding().getVariable("event")
}
