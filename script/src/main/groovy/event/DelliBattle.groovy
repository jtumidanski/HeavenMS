...
class EventDelliBattle {
   EventManager em
   boolean isPq = true
   int minPlayers = 1, maxPlayers = 2
   int minLevel = 120, maxLevel = 255
   int entryMap = 925010300
   int exitMap = 925010200
   int recruitMap = 925010200
   int clearMap
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Delli" + lobbyid)
...
EventDelliBattle getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventDelliBattle(em: em))
   }
   return (EventDelliBattle) getBinding().getVariable("event")
}
