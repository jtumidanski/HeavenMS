   ...
   def start() {
      ...
      if (!YamlConfig.config.server.USE_CPQ) {
         if (YamlConfig.config.server.USE_ENABLE_CUSTOM_NPC_SCRIPT) {
            ...
            cm.sendOk(I18nMessage.from("2042001_CARNIVAL_UNAVAILABLE"))
   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (cm.getPlayer().getMapId() == 980000010) {
            if (status == 0) {
               cm.sendNext(I18nMessage.from("2042001_HOPE_YOU_HAD_FUN"))
            ...
            if (status == 0) {
               if (cm.getParty() == null) {
                  status = 10
                  cm.sendOk(I18nMessage.from("2042001_MUST_CREATE_A_PARTY"))

               } else if (!cm.isLeader()) {
                  status = 10
                  cm.sendOk(I18nMessage.from("2042001_LEADER_MUST_START"))
                  ...
                  if (party >= 1) {
                     status = 10
                     cm.sendOk(I18nMessage.from("2042001_NOT_ENOUGH_PEOPLE").with(cpqMinAmt, cpqMaxAmt))

                  } else if (lvlOk != inMap) {
                     status = 10
                     cm.sendOk(I18nMessage.from("2042001_LEVEL_RANGE").with(cpqMinLvl, cpqMaxLvl))

                  } else if (isOutMap > 0) {
                     status = 10
                     cm.sendOk(I18nMessage.from("2042001_PARTY_MEMBERS_NOT_IN_MAP"))

                  } else {
                     if (!cm.sendCPQMapLists()) {
                        cm.sendOk(I18nMessage.from("2042001_FIELDS_ARE_ALL_FULL"))
               ...
               if (cm.fieldTaken(selection)) {
                  if (cm.fieldLobbied(selection)) {
                     ...
                     cm.sendOk(I18nMessage.from("2042001_ROOM_IS_FULL"))
                  ...
                  if ((selection >= 0 && selection <= 3) && party.size() < (YamlConfig.config.server.USE_ENABLE_SOLO_EXPEDITIONS ? 1 : 2)) {
                     cm.sendOk(I18nMessage.from("2042001_NEED_AT_LEAST_2_PLAYERS"))

                  } else if ((selection >= 4 && selection <= 5) && party.size() < (YamlConfig.config.server.USE_ENABLE_SOLO_EXPEDITIONS ? 1 : 3)) {
                     cm.sendOk(I18nMessage.from("2042001_NEED_AT_LEAST_3_PLAYERS"))
            ...
            if (status == 0) {
               ...
               if (selection == 0) {
                  if ((cm.getLevel() > 29 && cm.getLevel() < 51) || cm.getPlayer().isGM()) {
                     ...
                     cm.sendOk(I18nMessage.from("2042001_MUST_BE_AT_LEAST_LEVEL_30"))

                     cm.dispose()
                  } else {
                     cm.sendOk(I18nMessage.from("2042001_SORRY_LEVEL_RANGE"))

                     cm.dispose()
                  }
               } else if (selection == 1) {
                  status = 60
                  cm.sendSimple(I18nMessage.from("2042001_WHAT_WOULD_YOU_LIKE_TO_DO"))

               } else if (selection == 2) {
                  cm.sendSimple(I18nMessage.from("2042001_REMEMBER").with(n1, n2))
               ...
               if (select == 0) {
                  if (cm.haveItem(4001129, n1) && cm.canHold(4001129)) {
                     ...
                     cm.sendOk(I18nMessage.from("2042001_MISSING_SOMETHING_OR_EQUIP_IS_FULL"))
                  ...
                  if (cm.haveItem(4001129, n2) && cm.canHold(2041211)) {
                     ...
                     cm.sendOk(I18nMessage.from("2042001_MISSING_SOMETHING_OR_USE_IS_FULL"))

                     cm.dispose()
                  }
               } else if (select == 2) {//S2 Warrior 26 S3 Magician 6 S4 Bowman 6 S5 Thief 8
                  status = 10
                  cm.sendSimple(I18nMessage.from("2042001_PLEASE_MAKE_SURE").with(n3, n3, n4, n4, n5, n5, n3, n3, n4, n4, n5, n5))

               } else if (select == 3) {
                  status = 20
                  cm.sendSimple(I18nMessage.from("2042001_SELECT_THE_WEAPON_TO_TRADE").with(n3, n3, n4, n4, n5, n5))

               } else if (select == 4) {
                  status = 30
                  cm.sendSimple(I18nMessage.from("2042001_SELECT_THE_WEAPON_TO_TRADE_2").with(n3, n4, n5, n3, n4, n5))

               } else if (select == 5) {
                  status = 40
                  cm.sendSimple(I18nMessage.from("2042001_SELECT_THE_WEAPON_TO_TRADE_3").with(n3, n4, n5, n3, n4, n4, n5, n5))

               } else if (select == 6) {
                  status = 50 //pirate rewards
                  cm.sendSimple(I18nMessage.from("2042001_SELECT_THE_WEAPON_TO_TRADE_4").with(n3, n4, n5, n3, n4, n5))

               }
            } else if (status == 11) {
               if (selection == 12) {
                  cm.sendSimple(I18nMessage.from("2042001_SELECT_THE_WEAPON_TO_TRADE_5").with(n3, n3, n4, n4, n5, n5, n3, n3, n4, n4, n5, n5))
                  ...
                  if (cm.haveItem(4001129, cost[selection]) && cm.canHold(item[selection])) {
                     ...
                     cm.sendOk(I18nMessage.from("2042001_NOT_ENOUGH_OR_INVENTORY_IS_FULL"))
               ...
               if (selection == 12) {
                  status = 10
                  cm.sendSimple(I18nMessage.from("2042001_MAKE_SURE_2").with(n3, n3, n4, n4, n5, n5, n3, n3, n4, n4, n5, n5))
               ...
               select = selection
               if (selection == 0) {
                  cm.sendNext(I18nMessage.from("2042001_HELLO"))

               } else if (selection == 1) {
                  cm.sendNext(I18nMessage.from("2042001_MONSTER_CARNIVAL_INFO"))

               } else if (selection == 2) {
                  cm.sendNext(I18nMessage.from("2042001_EASY_RIGHT"))
               ...
               if (select == 0) {
                  cm.sendNext(I18nMessage.from("2042001_WHAT_IS_IT"))

               } else if (select == 1) {
                  cm.sendNext(I18nMessage.from("2042001_HOW_TO"))

               } else if (select == 2) {
                  cm.sendNext(I18nMessage.from("2042001_COMMANDS"))

               }
            } else if (status == 63) {
               if (select == 0) {
                  cm.sendNext(I18nMessage.from("2042001_I_KNOW_IT_IS_TOO_DANGEROUS"))

               } else if (select == 1) {
                  cm.sendNext(I18nMessage.from("2042001_3_WAYS_TO_DISTRACT"))

               } else if (select == 2) {
                  cm.sendNext(I18nMessage.from("2042001_SUMMONING"))

               }
            } else if (status == 64) {
               if (select == 0) {
                  cm.sendNext(I18nMessage.from("2042001_OF_COURSE"))

                  cm.dispose()
               } else if (select == 1) {
                  cm.sendNext(I18nMessage.from("2042001_PLEASE_REMEMBER"))

               } else if (select == 2) {
                  cm.sendNext(I18nMessage.from("2042001_ABILITY"))

               }
            } else if (status == 65) {
               if (select == 1) {
                  cm.sendNext(I18nMessage.from("2042001_DO_NOT_LOSE_EXP_WHEN_DEAD"))

                  cm.dispose()
               } else if (select == 2) {
                  cm.sendNext(I18nMessage.from("2042001_PROTECTOR"))

               }
            } else if (status == 66) {
               cm.sendNext(I18nMessage.from("2042001_CANNOT_USE_ITEMS"))
               ...
               if (allDone) {
                  cm.sendOk(I18nMessage.from("2042001_DONE"))

               } else {
                  cm.sendOk(I18nMessage.from("2042001_DONE_BUT_LACK_ETC_SPACE"))
