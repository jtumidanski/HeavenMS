   ...
   def processNanaQuest() {
      if (cm.haveItem(questItems[nanaLoc], 50)) {
         if (cm.canHold(4031367 + nanaLoc, 1)) {
            ...
            cm.sendOk(I18nMessage.from("9201025_THANK_YOU"))
            return true
         } else {
            cm.sendOk(I18nMessage.from("9201025_ETC_SPACE_NEEDED"))
         }
      } else {
         cm.sendOk(I18nMessage.from("9201025_GATHER_ME").with(questItems[nanaLoc]))
      }
      ...
      if (mode == -1) {
         ...
         if (status == 0) {
            if (!cm.isQuestStarted(100400)) {
               cm.sendOk(I18nMessage.from("9201025_HELLO"))
               cm.dispose()
               return
            }
            ...
            if (!cm.haveItem(4031367 + nanaLoc, 1)) {
               if (cm.isQuestCompleted(100401 + nanaLoc)) {
                  state = 1
                  cm.sendAcceptDecline(I18nMessage.from("9201025_DID_YOU_LOSE").with(questItems[nanaLoc]))
                  ...
                  state = 0
                  cm.sendAcceptDecline(I18nMessage.from("9201025_SEARCHING_FOR"))
               }
            } else {
               cm.sendOk(I18nMessage.from("9201025_DID_YOU_GET"))
               cm.dispose()
            }
         } else if (status == 1) {
            if (state == 0) {
               ...
               cm.sendOk(I18nMessage.from("9201025_COLLECT").with(questItems[nanaLoc]))
               cm.dispose()
            } else {
               processNanaQuest()
               cm.dispose()
