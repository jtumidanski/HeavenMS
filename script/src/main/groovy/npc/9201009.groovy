...
class NPC9201009 {
   ...
   def start() {
      ...
      if (cm.getMapId() == 680000200) {
         if (eim.getIntProperty("weddingStage") == 0) {
            cm.sendNext(I18nMessage.from("9201009_GUESTS_ARE_GATHERING"))
         } else {
            cm.warp(680000210, "sp")
            cm.sendNext(I18nMessage.from("9201009_PICK_YOUR_SEAT"))
         }

         cm.dispose()
      } else {
         if (cm.getPlayer().getId() != eim.getIntProperty("groomId") && cm.getPlayer().getId() != eim.getIntProperty("brideId")) {
            cm.sendNext(I18nMessage.from("9201009_NO_TALKING_TO_ME"))
            cm.dispose()
            return
         }
   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1 || mode == 0) {
         cm.sendOk(I18nMessage.from("9201009_GOOD_BYE"))
         cm.dispose()
         return
      } else if (mode == 1) {
         status++
      ...
      if (status == 0) {
         ...
         if (hasGoldenLeaf && hasEngage) {
            cm.sendOk(I18nMessage.from("9201009_YOU_CANNOT_LEAVE_YET"))
            cm.dispose()
            ...
            cm.sendNext(I18nMessage.from("9201009_MUST_NOT_BELONG"))
         }
      } else if (status == 1) {
         ...
         switch (selection) {
            case 0:
               if (eim.getIntProperty("isPremium") == 1) {
                  ...
                  cm.sendOk(I18nMessage.from("9201009_CHERISH_YOUR_PHOTOS"))
                  if (cmPartner != null) {
                  ...
                  cm.sendOk(I18nMessage.from("9201009_CONGRATULATIONS"))
                  if (cmPartner != null) {
            ...
            case 1:
               cm.sendOk(I18nMessage.from("9201009_GO_TO_THE_AFTER_PARTY"))
               cm.dispose()
               break
