   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (eim.getProperty(stage.toString() + "stageclear") != null) {
            cm.sendNext(I18nMessage.from("2040036_GO_TO_NEXT_STAGE"))
         } else {
            if (eim.isEventLeader(cm.getPlayer())) {
               int state = eim.getIntProperty("statusStg" + stage)

               if (state == -1) {           // preamble
                  cm.sendOk(I18nMessage.from("2040036_WELCOME").with(stage))
                  eim.setProperty("statusStg" + stage, 0)
               } else {       // check stage completion
                  if (cm.haveItem(4001022, 25)) {
                     cm.sendOk(I18nMessage.from("2040036_GOOD_JOB"))
                     cm.gainItem(4001022, (short) -25)

                     eim.setProperty("statusStg" + stage, 1)
                     clearStage(stage, eim, curMap)
                  } else {
                     cm.sendNext(I18nMessage.from("2040036_SORRY"))
                  }
               }
            } else {
               cm.sendNext(I18nMessage.from("2040036_PARTY_LEADER_MUST_TALK"))
            }
         }
