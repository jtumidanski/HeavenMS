...
class NPC1052001 {
   ...
   def actionx = ["1stJob": false, "2ndjob": false, "3thJobI": false, "3thJobC": false]
   int job = 410
   ...
   def start() {
      if ((cm.getJobId() / 100).intValue() == jobType && cm.canSpawnPlayerNpc(GameConstants.getHallOfFameMapid(cm.getJob()))) {
         ...
         } else if (actionx["3thJobI"] || (cm.getPlayer().gotPartyQuestItem("JB3") && cm.getLevel() >= 70 && cm.getJobId() % 10 == 0 && (cm.getJobId() / 100).intValue() == 4 && !cm.getPlayer().gotPartyQuestItem("JBP"))) {
