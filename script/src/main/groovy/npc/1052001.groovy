...
class NPC1052001 {
   ...
   def start() {
      if ((cm.getJobId() / 100).intValue() == jobType && cm.canSpawnPlayerNpc(GameConstants.getHallOfFameMapId(cm.getJob()))) {
         ...
         if (cm.getJobId() == 0) {
            action["1stJob"] = true
            cm.sendNext("Want to be a #rthief#k? There are some standards to meet. because we can't just accept EVERYONE in... #bYour level should be at least 10, with at least your " + cm.getFirstJobStatRequirement(jobType) + "#k. Let's see.")
   ...
   def action(Byte mode, Byte type, Integer selection) {
      ...
      if (action["1stJob"]) {
         ...
         if (status == 0) {
            ...
            job += selection * 10
            cm.sendYesNo("So you want to make the second job advancement as the " + (job == 410 ? "#bAssassin#k" : "#bBandit#k") + "? You know you won't be able to choose a different job for the 2nd job advancement once you make your decision here, right?")
         } else if (status == 3) {
            if (cm.haveItem(4031012)) {
            ...
            if (job == 410) {
               cm.sendNext("Alright, from here on out you are the #bAssassin#k. Assassins have quick hands and quicker feet to dominate the enemies. Please keep training. I'll make you even more powerful than you are right now!")
            } else {
