...
class NPC1002000 {
   ...
   def action(Byte mode, Byte type, Integer selection) {
      ...
      if (status == 1) {
         ...
         if (selection == 0) {
            town = true
            def text = "There are 7 big towns here in Victoria Island. Which of those do you want to know more of?#b"
            for (def i = 0; i < iMaps.length; i++) {
               text += "\r\n#L" + i + "##m" + iMaps[i] + "##l"
            }
            cm.sendSimple(text)
         } else if (selection == 1) {
            def selStr = cm.getJobId() == 0 ? "There's a special 90% discount for all beginners. Alright, where would you want to go?#b" : "Oh you aren't a beginner, huh? Then I'm afraid I may have to charge you full price. Where would you like to go?#b"
         ...
         if (status == 3) {
            ...
            townText[selectedMap][status - 3] == null ? cm.dispose() : cm.sendNextPrev(townText[selectedMap][status - 3])
         }
      } else if (status == 3) {
         selectedMap = selection
