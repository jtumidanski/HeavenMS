...
class NPC2131003 {
   ...
   def action(Byte mode, Byte type, Integer selection) {
      ...
      if (status == 0) {
         cm.sendSimple(I18nMessage.from("2131003_SO_MANY_INJURIES"))
      } else if (status == 1) {
         if (!cm.haveItem(exchangeItem, 100)) {
            cm.sendNext(I18nMessage.from("2131003_NEED_AT_LEAST_100"))
            cm.dispose()
         } else {
            double itemQuantity = cm.itemQuantity(exchangeItem) / 100
            cm.sendGetNumber(I18nMessage.from("2131003_PERFECT_PITCH").with(exchangeItem, exchangeItem))
         }
      } else if (status == 2) {
         if (selection >= 1 && selection <= cm.itemQuantity(exchangeItem) / 100) {
            if (!cm.canHold(4310000, selection)) {
               cm.sendOk(I18nMessage.from("2131003_MAKE_SOME_ETC_SPACE"))
            } else {
               cm.gainItem(4310000, (short) selection)
               cm.gainItem(exchangeItem, (short) -(selection * 100))
               cm.sendOk(I18nMessage.from("2131003_THANKS"))
            }
         }
         cm.dispose()
      }
