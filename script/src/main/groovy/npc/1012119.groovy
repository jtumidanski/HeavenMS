...
class NPC1012119 {
   ...
   def action(Byte mode, Byte type, Integer selection) {
      ...
      if (status == 0) {
         if (cm.getLevel() >= 20) {
            cm.sendOk(I18nMessage.from("1012119_UNDER_20_ONLY"))
            cm.dispose()
         } else if (cm.isQuestActive(22515) || cm.isQuestActive(22516) || cm.isQuestActive(22517) || cm.isQuestActive(22518)) {
            cm.sendYesNo(I18nMessage.from("1012119_WOULD_YOU_LIKE_TO_GO_SPECIAL"))
            status = 1
         } else {
            String selStr = ""
            for (def i = 0; i < num; i++) {
            ...
            cm.sendSimple(I18nMessage.from("1012119_WOULD_YOU_LIKE_TO_GO").with(selStr))
         }
         ...
         if (selection < 0 || selection >= num) {
            ...
            cm.sendNext(I18nMessage.from("1012119_MAP_FULL"))
            status = -1
         } else {
