      ...
      if (mode == -1) {
         ...
         if (!weddingIndoors) {
            ...
            if (status == 0) {
               ...
               switch (selection) {
                  case 0:
                     cm.sendOk(I18nMessage.from("9201008_ENGAGEMENT_INFO").with(weddingEntryTicketCommon))
                     cm.dispose()
                     break
                  case 1:
                     if (hasEngagement) {
                        ...
                        if (weddingId > 0) {
                           if (channel.isWeddingReserved(weddingId)) {    // registration check
                              ...
                              cm.sendOk(I18nMessage.from("9201008_WEDDING_TIME").with(placeTime))
                           } else {
                              MapleCharacter partner = world.getPlayerStorage().getCharacterById(cm.getPlayer().getPartnerId()).get()
                              if (partner == null) {
                                 cm.sendOk(I18nMessage.from("9201008_PARTNER_OFFLINE"))
                                 cm.dispose()
                                 return
                              }

                              if (hasWeddingRing(cm.getPlayer()) || hasWeddingRing(partner)) {
                                 cm.sendOk(I18nMessage.from("9201008_YOU_OR_PARTNER_HAS_MARRIAGE_RING_ALREADY"))
                                 cm.dispose()
                                 return
                              }

                              if (cm.getMap() != partner.getMap()) {
                                 cm.sendOk(I18nMessage.from("9201008_PARNTER_MUST_REGISTER"))
                                 cm.dispose()
                                 return
                              }

                              if (!cm.canHold(weddingSendTicket, 15) || !partner.canHold(weddingSendTicket, 15)) {
                                 cm.sendOk(I18nMessage.from("9201008_YOU_OR_PARTNER_NEED_ETC_SPACE"))
                                 cm.dispose()
                                 return
                              }

                              if (!cm.getUnclaimedMarriageGifts().isEmpty() || !partner.getAbstractPlayerInteraction().getUnclaimedMarriageGifts().isEmpty()) {
                                 cm.sendOk(I18nMessage.from("9201008_SOMETHING_DOES_NOT_SEEM_RIGHT"))
                                 cm.dispose()
                                 return
                              }
                              ...
                              if (hasCommon || hasPremium) {
                                 ...
                                 if (resStatus > 0) {
                                    ...
                                    cm.sendOk(I18nMessage.from("9201008_RECEIVED_TICKETS").with(wedType, placeTime))
                                    ...
                                    cm.sendOk(I18nMessage.from("9201008_PLEASE_TRY_AGAIN_LATER"))
                                 }
                              } else {
                                 cm.sendOk(I18nMessage.from("9201008_BEFORE_TRYING_TO_REGISTER").with(weddingEntryTicketCommon))
                              }
                           }
                        } else {
                           cm.sendOk(I18nMessage.from("9201008_ENCOUNTERED_ERROR"))
                        }

                        cm.dispose()
                     } else {
                        cm.sendOk(I18nMessage.from("9201008_NEED_ENGAGEMENT_RING"))
                        cm.dispose()
                     }
                     break
                     ...
                     if (cm.haveItem(weddingGuestTicket)) {
                        ...
                        if (wid > 0) {
                           if (channel.isOngoingWeddingGuest(cathedralWedding, cm.getPlayer().getId())) {
                              ...
                              if (eim != null) {
                                 cm.sendOk(I18nMessage.from("9201008_DO_NOT_DROP_GOLD_MAPLE_LEAF"))
                              } else {
                                 cm.sendOk(I18nMessage.from("9201008_PLEASE_WAIT"))
                                 cm.dispose()
                              }
                           } else {
                              cm.sendOk(I18nMessage.from("9201008_YOU_ARE_NOT_INVITED"))
                              cm.dispose()
                           }
                        } else {
                           cm.sendOk(I18nMessage.from("9201008_NO_WEDDING_BOOKED"))
                           cm.dispose()
                        }
                     } else {
                        cm.sendOk(I18nMessage.from("9201008_NEED_TICKET").with(weddingGuestTicket))
                        cm.dispose()
                     }
                     break
                  default:
                     ...
                     if (resStatus > 0) {
                        if (cm.canHold(weddingSendTicket, 3)) {
                           ...
                           cm.sendOk(I18nMessage.from("9201008_NEED_ETC_SPACE"))
                        }
                     } else {
                        cm.sendOk(I18nMessage.from("9201008_NOT_CURRENTLY_BOOKED"))
                     }
                     cm.dispose()
               }
               ...
               if (eim != null) {
                  ...
                  cm.sendOk(I18nMessage.from("9201008_MARRIAGE_EVENT_COULD_NOT_BE_FOUND"))
               }
            ...
            if (status == 0) {
               ...
               if (eim.getIntProperty("weddingStage") == 0) {
                  if (!isMarrying) {
                     cm.sendOk(I18nMessage.from("9201008_WELCOME").with(cm.getMapId()))
                  } else {
                     cm.sendOk(I18nMessage.from("9201008_WELCOME_SHORT").with(cm.getMapId()))
                  }

                  cm.dispose()
               } else {
                  cm.sendYesNo(I18nMessage.from("9201008_BRIDE_AND_GROOM_ARE_READY"))
               }
            } else if (status == 1) {
