...
class EventAreaBossBamboo {
   ...
   def start() {
      MapleMap mapObj = em.getChannelServer().getMapFactory().getMap(800020120)   // original mapid was 251010101
      MapleMonster mobObj = MapleLifeFactory.getMonster(6090002)
...
EventAreaBossBamboo getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossBamboo(em: em))
   }
   return (EventAreaBossBamboo) getBinding().getVariable("event")
}
