...
class EventMahaBattle {
   EventManager em
   boolean isPq = true
   int minPlayers, maxPlayers
   int minLevel, maxLevel
   int entryMap = 914020000
   int exitMap = 140000000
   int recruitMap = 140000000
   int clearMap = 140000000
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Maha" + lobbyid)
...
EventMahaBattle getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventMahaBattle(em: em))
   }
   return (EventMahaBattle) getBinding().getVariable("event")
}
