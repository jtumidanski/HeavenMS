...
class EventAreaBossSeruf {
   ...
   def start() {
      MapleMap theSeaweedTower = em.getChannelServer().getMapFactory().getMap(230020100)
      MapleMonster seruf = MapleLifeFactory.getMonster(4220001)
...
EventAreaBossSeruf getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossSeruf(em: em))
   }
   return (EventAreaBossSeruf) getBinding().getVariable("event")
}
