...
class EventElementalBattle {
   EventManager em
   boolean isPq = true
   int minPlayers = 2, maxPlayers = 2
   int minLevel = 100, maxLevel = 255
   int entryMap = 922020100
   int exitMap = 220050300
   int recruitMap = 220050300
   int clearMap = 220050300
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Elemental" + lobbyid)
   ...
   static def monsterKilled(MapleMonster mob, EventInstanceManager eim) {
      if (isElemental(mob)) {
         int killed = eim.getIntProperty("boss")
         if (killed == 1) {
            eim.showClearEffect()
            eim.clearPQ()
         } else {
...
EventElementalBattle getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventElementalBattle(em: em))
   }
   return (EventElementalBattle) getBinding().getVariable("event")
}
