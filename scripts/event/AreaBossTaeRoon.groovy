...
class EventAreaBossTaeRoon {
   ...
   def start() {
      MapleMap territoryOfWanderingBear = em.getChannelServer().getMapFactory().getMap(250010304)
      MapleMonster taeRoon = MapleLifeFactory.getMonster(7220000)
...
EventAreaBossTaeRoon getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossTaeRoon(em: em))
   }
   return (EventAreaBossTaeRoon) getBinding().getVariable("event")
}
