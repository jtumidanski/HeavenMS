...
class EventTD_Battle3 {
   EventManager em
   boolean isPq = true
   int minPlayers = 2, maxPlayers = 6
   int minLevel = 70, maxLevel = 255
   int entryMap = 240070403
   int exitMap = 240070402
   int recruitMap = 240070402
   int clearMap = 240070402
   int minMapId = 240070403
   int maxMapId = 240070403
   int eventTime = 15
   int[] lobbyRange = [0, 0]

   int tdBossId = 8220011
...
EventTD_Battle3 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventTD_Battle3(em: em))
   }
   return (EventTD_Battle3) getBinding().getVariable("event")
}
