...
class EventCabin {
   ...
   MapleMap Orbis_btf
   MapleMap Leafre_btf
   MapleMap Cabin_to_Orbis
   MapleMap Cabin_to_Leafre
   MapleMap Orbis_docked
   MapleMap Leafre_docked
   MapleMap Orbis_Station
   MapleMap Leafre_Station
...
EventCabin getEvent() {
   if (!getBinding().hasVariable("event")) {
      ...
      getBinding().setVariable("event", new EventCabin(em: em))
   }
   return (EventCabin) getBinding().getVariable("event")
}
