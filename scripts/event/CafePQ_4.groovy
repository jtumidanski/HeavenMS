...
class EventCafePQ_4 {
   ...
   int entryMap = 195000000
   int exitMap = 193000000
   int recruitMap = 193000000
   int clearMap
   int minMapId = 195000000
   int maxMapId = 195030000
   int eventTime = 45
   int[] lobbyRange = [0, 0]

   int couponsNeeded = 450
   int[] eventMaps = [195000000, 195010000, 195020000, 195030000]
   ...
   static def setEventRewards(EventInstanceManager eim) {
      ...
      List<Integer> itemSet = [4001011]
      List<Integer> itemQty = [1]
      eim.setEventRewards(evLevel, itemSet, itemQty)

      List<Integer> expStages = [21000]    //bonus exp given on CLEAR stage signal
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Lan4_" + lobbyid)
...
EventCafePQ_4 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventCafePQ_4(em: em))
   }
   return (EventCafePQ_4) getBinding().getVariable("event")
}
