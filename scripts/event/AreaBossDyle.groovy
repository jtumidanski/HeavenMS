...
class EventAreaBossDyle {
   ...
   def start() {
      MapleMap dangeroudCroko1 = em.getChannelServer().getMapFactory().getMap(107000300)
      ...
      MessageBroadcaster.getInstance().sendMapServerNotice(dangeroudCroko1, ServerNoticeType.LIGHT_BLUE, "The huge crocodile Dyle has come out from the swamp.")
      setupTask = em.schedule("start", 3 * 60 * 60 * 1000)
   }
...
EventAreaBossDyle getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossDyle(em: em))
   }
   return (EventAreaBossDyle) getBinding().getVariable("event")
}
