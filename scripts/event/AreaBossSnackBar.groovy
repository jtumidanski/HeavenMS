...
class EventAreaBossSnackBar {
   ...
   def start() {
      MapleMap snackBarMap = em.getChannelServer().getMapFactory().getMap(105090310)
      MapleMonster snackBar = MapleLifeFactory.getMonster(8220008)
...
EventAreaBossSnackBar getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossSnackBar(em: em))
   }
   return (EventAreaBossSnackBar) getBinding().getVariable("event")
}
