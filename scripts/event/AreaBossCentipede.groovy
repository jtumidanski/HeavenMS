...
class EventAreaBossCentipede {
   ...
   def start() {
      MapleMap herbGarden = em.getChannelServer().getMapFactory().getMap(251010102)
      MapleMonster gcent = MapleLifeFactory.getMonster(5220004)
...
EventAreaBossCentipede getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossCentipede(em: em))
   }
   return (EventAreaBossCentipede) getBinding().getVariable("event")
}
