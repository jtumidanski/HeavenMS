...
class EventAreaBossMano {
   ...
   def start() {
      MapleMap thicketAroundTheBeach3 = em.getChannelServer().getMapFactory().getMap(104000400)
      MapleMonster mano = MapleLifeFactory.getMonster(2220000)
...
EventAreaBossMano getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossMano(em: em))
   }
   return (EventAreaBossMano) getBinding().getVariable("event")
}
