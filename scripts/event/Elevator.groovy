...
class EventElevator {
   ...
   int beginTime = 60 * 1000 //The time to begin the ride
   int rideTime = 60 * 1000 //The time that require move to destination
...
EventElevator getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventElevator(em: em))
   }
   return (EventElevator) getBinding().getVariable("event")
}
...
def goingUpNow(EventInstanceManager eim) {
   getEvent().goingUpNow()
}

def goingDownNow(EventInstanceManager eim) {
   getEvent().goingDownNow()
}

def isUpNow(EventInstanceManager eim) {
   getEvent().isUpNow()
}

def isDownNow(EventInstanceManager eim) {
   getEvent().isDownNow()
}