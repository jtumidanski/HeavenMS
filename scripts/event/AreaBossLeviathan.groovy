...
class EventAreaBossLeviathan {
   ...
   def start() {
      MapleMap leviathansCanyon = em.getChannelServer().getMapFactory().getMap(240040401)
      MapleMonster leviathan = MapleLifeFactory.getMonster(8220003)
...
EventAreaBossLeviathan getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossLeviathan(em: em))
   }
   return (EventAreaBossLeviathan) getBinding().getVariable("event")
}
