...
class EventHak {
   ...
   int[] returnTo = [200000141, 250000100]
   int[] rideTo = [250000100, 200000141]
   int[] birdRide = [200090300, 200090310]
   int myRide
   MapleMap returnMap
   int map
   MapleMap onRide
   int rideTime = 60 * 1000
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("Hak_" + lobbyid)
      return eim
   }
...
EventHak getEvent() {
   if (!getBinding().hasVariable("event")) {
      ...
      getBinding().setVariable("event", new EventHak(em: em))
   }
   return (EventHak) getBinding().getVariable("event")
}
