...
class EventAreaBossKimera {
   ...
   def start() {
      MapleMap labSecretBasementPath = em.getChannelServer().getMapFactory().getMap(261030000)
      MapleMonster chimera = MapleLifeFactory.getMonster(8220002)
...
EventAreaBossKimera getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossKimera(em: em))
   }
   return (EventAreaBossKimera) getBinding().getVariable("event")
}
