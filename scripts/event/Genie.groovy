...
class EventGenie {
   ...
   MapleMap Orbis_btf
   MapleMap Genie_to_Orbis
   MapleMap Orbis_docked
   MapleMap Ariant_btf
   MapleMap Genie_to_Ariant
   MapleMap Ariant_docked
...
EventGenie getEvent() {
   if (!getBinding().hasVariable("event")) {
      ...
      getBinding().setVariable("event", new EventGenie(em: em))
   }
   return (EventGenie) getBinding().getVariable("event")
}
