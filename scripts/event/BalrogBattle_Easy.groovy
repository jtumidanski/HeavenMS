...
class EventBalrogBattle_Easy {
   EventManager em
   boolean isPq = true
   int minPlayers = 3, maxPlayers = 30
   int minLevel = 50, maxLevel = 255
   int entryMap = 105100400
   int exitMap = 105100100
   int recruitMap = 105100100
   int clearMap = 105100401
   int minMapId = 105100400
   int maxMapId = 105100401
   int eventTime = 60
   int[] lobbyRange = [0, 0]

   int minMobId = 8830007
   int maxMobId = 8830013
   int bossMobId = 8830010
   ...
   static def isUnsealedBalrog(MapleMonster mob) {
      int balrogid = mob.getId() - 8830007
...
EventBalrogBattle_Easy getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventBalrogBattle_Easy(em: em))
   }
   return (EventBalrogBattle_Easy) getBinding().getVariable("event")
}
