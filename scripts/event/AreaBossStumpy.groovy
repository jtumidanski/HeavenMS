...
class EventAreaBossStumpy {
   ...
   def start() {
      MapleMap eastRockyMountain5 = em.getChannelServer().getMapFactory().getMap(101030404)
      MapleMonster stumpy = MapleLifeFactory.getMonster(3220000)
...
EventAreaBossStumpy getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossStumpy(em: em))
   }
   return (EventAreaBossStumpy) getBinding().getVariable("event")
}
