...
class Event4jaerial {
   ...
   int entryMap = 912020000
   int exitMap = 120000102
   int recruitMap
   int clearMap
   ...
   def setup(int level, int lobbyid) {
      EventInstanceManager eim = em.newInstance("4jaerial_" + lobbyid)
...
Event4jaerial getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new Event4jaerial(em: em))
   }
   return (Event4jaerial) getBinding().getVariable("event")
}
