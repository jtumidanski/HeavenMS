...
class EventAreaBossZeno {
   ...
   def start() {
      MapleMap graysPrairie = em.getChannelServer().getMapFactory().getMap(221040301)
      MapleMonster zeno = MapleLifeFactory.getMonster(6220001)
...
EventAreaBossZeno getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventAreaBossZeno(em: em))
   }
   return (EventAreaBossZeno) getBinding().getVariable("event")
}
