...
class EventTD_Battle1 {
   EventManager em
   boolean isPq = true
   int minPlayers = 2, maxPlayers = 6
   int minLevel = 70, maxLevel = 255
   int entryMap = 240070203
   int exitMap = 240070202
   int recruitMap = 240070202
   int clearMap = 240070202
   int minMapId = 240070203
   int maxMapId = 240070203
   int eventTime = 10
   int[] lobbyRange = [0, 0]

   int tdBossId = 7220005
...
EventTD_Battle1 getEvent() {
   if (!getBinding().hasVariable("event")) {
      EventManager em = (EventManager) getBinding().getVariable("em")
      getBinding().setVariable("event", new EventTD_Battle1(em: em))
   }
   return (EventTD_Battle1) getBinding().getVariable("event")
}
